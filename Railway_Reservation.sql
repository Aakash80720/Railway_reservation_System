...
CREATE FUNCTION public.reassign() RETURNS trigger
    ...
    AS $$
declare
    waiting_serialNo integer := 0;
    prev_coachNo integer := 0;
    prev_seatNo integer := 0;
    begin
        waiting_serialNo := (select "Not_confirm".serial_no
                             from "Not_confirm" order by time limit 1);
        prev_coachNo := (select p.coach_no from "Not_confirm" inner join passengers p on p.serial_no = "Not_confirm".serial_no);
        prev_seatNo := (select p.seat_no from "Not_confirm" inner join passengers p on p.serial_no = "Not_confirm".serial_no);
        if new.berth_status = true and waiting_serialNo != 0 and mod(new.seat_no,8) != 7 then
                update passengers set
                                      seat_no = new.seat_no,berth_status = 'CNF',
                                      berth_position = new."Berth_position",
                                      coach_no = new.coach_no
                ...
                update coach_table set "Max_count" = "Max_count"+1,berth_status = True where seat_no = prev_seatNo and coach_no = prev_coachNo;
...
CREATE FUNCTION public.update_status() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
    declare waiting integer := 0;
    BEGIN
        waiting := (select count(*) from "Not_confirm");
...
COPY public.coach_table (seat_no, berth_status, "Max_count", "Status", "Berth_position", coach_no) FROM stdin;
8	f	0	CNF	SU	1
15	t	2	RAC	SL	1
5	t	1	CNF	MB	1
4	t	1	CNF	LB	1
...
COPY public.passengers (serial_no, name, age, phone_number, seat_no, berth_position, berth_status, pnr_no, user_id, coach_no) FROM stdin;
81	RACtest3	43	9887654334	8	SU	CNF	8983626	2570584	1
72	Aakash	21	8072011083	1	LB	CNF	3253156	2570584	1
73	deepa	22	7072011083	2	MB	CNF	3253156	2570584	1
74	Deepak	20	9887654334	3	UP	CNF	3253156	2570584	1
\.
...
COPY public."user" (pnr_no, user_name, password, age, phone_number, isadmin) FROM stdin;
2570584	Aakash	Aakash@26	12	8072011083	f
4567895	Arun	Arun@15	80	7252585258	f
532	Aakash#Admin	Admin@26	20	8072011083	t
\.
...
SELECT pg_catalog.setval('public.coach_table_seat_no_seq', 16, true);
...
SELECT pg_catalog.setval('public.passengers_serial_no_seq', 81, true);
...
CREATE TRIGGER waiting_trigger AFTER INSERT OR UPDATE ON public.passengers FOR EACH ROW EXECUTE FUNCTION public.waiting_list();
